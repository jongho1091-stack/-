...
class RecruitModal(discord.ui.Modal, title='📝 레이드 모집'):
    t_in = discord.ui.TextInput(label='제목', placeholder='(ex.ㅇㅇ 4인파티)')
    tm_in = discord.ui.TextInput(label='출발 시간', placeholder='(ex.오늘 저녁 11시)')
    l_in = discord.ui.TextInput(label='인원', placeholder='(숫자만 적을 것)')
...
async def edit_setup_text(i):
    ...
    except: await i.response.send_message("❌ 수정 창을 열 수 없습니다.", ephemeral=True)
