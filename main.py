...
class RaidEntryModal(discord.ui.Modal, title='⚔️ 레이드 참석 정보 입력'):
    ...
    async def on_submit(self, i: discord.Interaction):
        if self.raid_view.is_closed: return await i.response.send_message("❌ 마감됨.", ephemeral=True)
        ...
        try:
            a = await i.channel.send(f"🔔 {self.raid_view.author.mention}님, **{self.char_name.value}** 참석"); await a.delete(delay=3)
        except: pass
...
class RaidView(discord.ui.View):
    ...
    async def leave(self, i, b):
        if i.user.id in self.roster:
            ...
            try:
                a = await i.channel.send(f"⚪ {self.author.mention}님, **{i.user.display_name}** 취소 (get off)"); await a.delete(delay=3)
            except: pass
    ...
    async def close(self, i, b):
        if i.user.id != self.author.id: return await i.response.send_message("❌ 작성자만 마감 가능", ephemeral=True)
    ...
    def get_embed(self, closed=False):
        ...
        embed.add_field(name="👥 현재 참여 명단", value=list_val, inline=False); return embed
...
class RecruitModal(discord.ui.Modal, title='📝 레이드 모집'):
    t_in = discord.ui.TextInput(label='제목')
    tm_in = discord.ui.TextInput(label='출발 시간')
    l_in = discord.ui.TextInput(label='인원')
...
class TicketView(discord.ui.View):
    ...
    async def create_ticket(self, i, prefix):
        ...
        emb = discord.Embed(title=f"📩 {prefix} 접수", description=f"{member.mention}님, 내용을 남겨주세요.\n\n⚠️ **3분 무응답 시 자동 폭파(get off)**", color=0x2f3136)
        emb.set_footer(text=f"로그채널ID: {self.log_ch_id}"); await ticket_ch.send(embed=emb)
        await i.response.send_message(f"✅ 티켓 생성! {ticket_ch.mention}", ephemeral=True)
        def check(m): return m.channel == ticket_ch and not m.author.bot
        while True:
            try: await bot.wait_for('message', check=check, timeout=180.0)
            except asyncio.TimeoutError:
                await ticket_ch.send("⏰ 3분간 응답이 없어 종료(get off)합니다."); await archive_and_delete(ticket_ch, self.log_ch_id); break
    @discord.ui.button(label="문의/건의하기", style=discord.ButtonStyle.success, emoji="🙋", custom_id="btn_inquiry_final_v10")
    async def inquiry(self, i, b): await self.create_ticket(i, "문의-건의")
    @discord.ui.button(label="신고하기", style=discord.ButtonStyle.danger, emoji="🚨", custom_id="btn_report_final_v10")
...
async def reg_role_cmd(i, 이모지:str, 역할명:str):
    ...
    save_db(bot.db)
    # [실시간 반영] 해당 채널에서 버튼 메시지를 찾아 즉시 업데이트
    await i.response.defer(ephemeral=True)
    async for m in i.channel.history(limit=50):
        if m.author == bot.user and m.components:
            await m.edit(view=RoleAssignView(bot, i.guild.id))
            return await i.followup.send(f"✅ 버튼 실시간 업데이트 완료!", ephemeral=True)
    await i.followup.send(f"✅ 등록 완료! (이 채널에 버튼 메시지가 없어 새로 만드셔야 합니다.)", ephemeral=True)
...
async def edit_role_msg(i, 수정내용:str):
    ...
    async for m in i.channel.history(limit=50):
        if m.author == bot.user and m.components:
            # [버튼 유지] View를 다시 넣어줘야 버튼이 안 사라짐
            await m.edit(content=수정내용, view=RoleAssignView(bot, i.guild.id))
            return await i.followup.send("✅ 문구 수정 완료!", ephemeral=True)
    await i.followup.send("❌ 대상을 못 찾음", ephemeral=True)
...
async def set_auto_role(i, 역할: discord.Role):
    ...
    save_db(bot.db); await i.response.send_message(f"✅ 자동 역할 설정됨", ephemeral=True)
...
async def ticket_setup(i, 관리자역할: discord.Role, 상담카테고리명: str, 로그채널명: str):
    ...
    bot.add_view(view); await i.channel.send(embed=discord.Embed(title="📢 문의 및 신고 접수", description="아래 버튼을 눌러 티켓을 생성하세요."), view=view)
...
async def close_ticket(i):
    ...
    await i.response.send_message("🏁 종료 중...")
    log_id = None
