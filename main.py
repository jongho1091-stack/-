...
import re
import os  # 환경 변수를 읽어오기 위해 필요합니다

# --- 설정 구간 ---
# 토큰은 이제 코드에 직접 넣지 않고 Koyeb 설정에서 넣을 것입니다.
ROLE_ID_LIST = [1466368581020422209] 
# -----------------
...
class RaidView(discord.ui.View):
    ...
    async def button_callback(self, interaction: discord.Interaction):
        ...
        is_full = sum(len(self.roster[r]) for r in self.roles) >= self.limit
        for child in self.children:
            if isinstance(child, discord.ui.Button) and child.label in self.roles:
...
class RecruitModal(discord.ui.Modal, title='📝 레이드 모집 작성'):
    ...
    limit_in = discord.ui.TextInput(label='모집 인원', placeholder='숫자만 입력 (예: 8)')
    async def on_submit(self, interaction: discord.Interaction):
        try:
            val = re.sub(r'[^0-9]', '', self.limit_in.value)
            if not val: return await interaction.response.send_message("숫자를 입력해주세요!", ephemeral=True)
            view = RaidView(self.title_in.value, self.time_in.value, int(val))
            mentions = " ".join([f"<@&{rid}>" for rid in ROLE_ID_LIST if rid != 0])
            await interaction.response.send_message(content=f"{mentions}\n🌲 **레이드 모집이 시작되었습니다!**", embed=view.get_embed(), view=view)
...
class ScheduleModal(discord.ui.Modal, title='📅 일정 체크 작성'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(content="📅 **일정 확인 부탁드립니다!**", embed=view.get_embed(), view=view)
